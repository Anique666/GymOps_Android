...
interface ReportsDao {
    ...
    @Query("SELECT COUNT(*) FROM members WHERE joinDate BETWEEN :startTime AND :endTime")
    fun observeNewMembersCount(startTime: Long, endTime: Long): Flow<Int>

    @Query("SELECT COUNT(*) FROM members WHERE expiryDate BETWEEN :startTime AND :endTime")
    fun observeExpiredMembersCount(startTime: Long, endTime: Long): Flow<Int>

    @Query("SELECT COUNT(*) FROM members WHERE expiryDate BETWEEN :now AND :threshold")
    ...
    @Query("SELECT dateOfBirth FROM members WHERE dateOfBirth > 0")
